...
def load_data(dirname):
    ...
    for file in listfile:
        ...
        for text in textlist:
            ...
            #print(numbers)
            row=42*8#Remove the first eight frames
            landmark_frame=[]
            for i in range(0,100):#Fixed to 100 frames total
                #print(numbers[row*42:(row*42)+41])
                landmark_frame.extend(numbers[row:row+42])
                row += 42
            landmark_frame=np.array(landmark_frame)
            landmark_frame=landmark_frame.reshape(-1,42)#Convert to Two Dimensions (260 * 42)
...
def load_label():
    label = {}
    count = 1
    listfile=['Computer','Deutschland','Hallo','Welt']
    for l in listfile:
        if "_" in l:
            continue
        label[l] = count
